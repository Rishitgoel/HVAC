import{f as b,h as u,g as f}from"./storage-BgqC2MNh.js";import{c as y,f as e}from"./calculations-DXcxGb77.js";import{g as o}from"./index-CteE1Qgu.js";const w=()=>`
...
`,I=async p=>{var l,c,m;const r=p.split("/"),i=r[1],d=r[2];document.getElementById("login-btn").addEventListener("click",()=>{o()?window.location.hash="#dashboard":window.location.hash="#login"}),document.getElementById("download-pdf-btn").addEventListener("click",()=>{window.print()});try{const s=await b(),a=await u(i,d),n=await f(i);if(!a||!n){document.getElementById("pdf-content").innerHTML='<div class="p-8 text-center text-error font-headline-md">Quote not found or you do not have permission to view it.</div>';return}if(a.status!=="published"&&(!o()||a.ownerUid!==o().uid&&!isAdmin())){document.getElementById("pdf-content").innerHTML='<div class="p-8 text-center text-error font-headline-md">This quote is not public.</div>';return}const t=y(a,s);document.getElementById("quote-date").textContent=`Date: ${new Date(((l=a.updatedAt)==null?void 0:l.toDate())||new Date).toLocaleDateString("en-IN")}`,document.getElementById("client-name-display").textContent=n.clientName,document.getElementById("project-title-display").textContent=n.title,document.getElementById("sheet-id-display").textContent=`Ref: ${d.split("-")[1]||d}`,document.getElementById("cfm-display").textContent=((c=a.clientInfo)==null?void 0:c.cfmRequirement)||0,document.getElementById("room-display").textContent=((m=a.clientInfo)==null?void 0:m.roomName)||"General";const x=document.getElementById("breakdown-body");x.innerHTML=`
    ...
    `,document.getElementById("subtotal-display").textContent=e(t.subtotal),document.getElementById("tax-label").textContent=`Tax (${s.taxRate||18}%)`,document.getElementById("tax-display").textContent=e(t.tax),document.getElementById("total-display").textContent=e(t.total)}catch(s){console.error(s)}};export{I as mount,w as render};
