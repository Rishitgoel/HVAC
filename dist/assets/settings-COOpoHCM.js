import{m as b,t as p,s as l,r as f}from"./toast-Ds53AZIo.js";import{f as m,s as v}from"./storage-CjZL2NYH.js";import{i as d,j as x}from"./index-CDlMNROc.js";const F=()=>`
  ${f("#settings")}
...
`,P=async()=>{b();const n=document.getElementById("mobile-menu-btn");n&&n.addEventListener("click",p);const i=x(),c=document.getElementById("settings-form"),a=document.getElementById("save-settings-btn"),u=document.getElementById("read-only-banner");i?a.classList.remove("hidden"):(u.classList.remove("hidden"),c.querySelectorAll("input, select").forEach(r=>{r.disabled=!0,r.classList.add("bg-surface-container-low","text-on-surface-variant","cursor-not-allowed")}));try{const e=await m();["unitSystem","taxRate","aluminumRate","giRate","puffPanelRate","forwardFanPrice","backwardFanPrice","plugFanPrice","ecFanPrice","coilRate","brownPadRate","greenPadRate","preFilterPrice","fineFilterPrice","hepaFilterPrice"].forEach(t=>{const s=document.getElementById(t);s&&e[t]!==void 0&&(s.value=e[t])}),o(e.unitSystem||"sqft")}catch(e){l("Error loading settings: "+d(e),"error")}document.getElementById("unitSystem").addEventListener("change",e=>o(e.target.value));function o(e){const r=e==="sqm"?"sq.m":"sq.ft";document.querySelectorAll(".unit-label").forEach(t=>{t.textContent=r})}i&&a.addEventListener("click",async()=>{a.disabled=!0,a.innerHTML='<div class="spinner spinner-sm"></div>';const e={};["unitSystem","taxRate","aluminumRate","giRate","puffPanelRate","forwardFanPrice","backwardFanPrice","plugFanPrice","ecFanPrice","coilRate","brownPadRate","greenPadRate","preFilterPrice","fineFilterPrice","hepaFilterPrice"].forEach(t=>{const s=document.getElementById(t).value;e[t]=t==="unitSystem"?s:parseFloat(s)||0});try{await v(e),l("Settings saved successfully!")}catch(t){l("Error saving settings: "+d(t),"error")}finally{a.disabled=!1,a.innerHTML='<span class="material-symbols-outlined text-[18px]">save</span><span class="hidden md:inline">Save Changes</span>'}})};export{P as mount,F as render};
